...
#pragma unused (target)
    ...
    tst *reachability = ((__bridge tst*)info);
#else
    tst *reachability = ((Reachability*)info);
#endif
...
{
    if ([[tst reachabilityForLocalWiFi] currentReachabilityStatus] != NotReachable) {
        return YES;
    }
    if ([[tst reachabilityForInternetConnection] currentReachabilityStatus] != NotReachable) {
        return YES;
    }
    return NO;
}
...
{
    return [tst reachabilityWithHostname:hostname];
}
